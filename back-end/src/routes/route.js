...
module.exports = function(app, dir, db) {
	app.post('/end', (req, res) => {
		let rawdata = fs.readFileSync(dir + '/config/default.json');
		fs.writeFileSync(dir + '/config/config.json', rawdata);
		db.dropCollection('playerdeets', function(err, result) {
			if (err) {
	...
	app.post('/loadCsv', (req, res) => {
		PlayerModel.find().then((doc) => {
			if (doc.length === 0) {
				fs
					.createReadStream(dir + '/public/csv/player.csv')
					.pipe(
						parse({
							delimiter: ','
						})
	...
	app.post('/addTeam', function(req, res) {
		TeamModel.find({ name: req.body.name }).then((doc) => {
			if (doc.length > 0) res.json({ done: false, message: `Team ${req.body.name} already exists` });
			else {
	...
	app.post('/bidDone', (req, res) => {
		TeamModel.findOne({ _id: req.body.tid }).then((team) => {
			PlayerModel.findOne({ _id: req.body.pid }).then((player) => {
				...
				if (player.type === 1) team.number.gk = team.number.gk + 1;
				else if (player.type === 2) team.number.mf = team.number.mf + 1;
				else if (player.type === 3) team.number.df = team.number.df + 1;
				else if (player.type === 4) team.number.fwd = team.number.fwd + 1;
	...
	app.post('/getTeams', (req, res) => {
		...
		PlayerModel.findOne({ _id: req.body.id }).then((pl) => {
			TeamModel.find().then((doc) => {
				var teams = [];
				doc.forEach((team) => {
					if (team.money <= pl.price) return;
					else if (team.number.total >= save.total) return;
					if (
						(pl.type === 1 && team.number.gk >= save.ngk) ||
						(pl.type === 2 && team.number.mf >= save.nmf) ||
						(pl.type === 3 && team.number.df >= save.ndf) ||
						(pl.type === 4 && team.number.fwd >= save.nfw)
					)
						return;
					else {
						teams.push(team);
